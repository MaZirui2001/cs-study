...
def create_del_result_frame(self, check_result_simple, check_result):
    ...
    for i in range(len(check_result_simple)):
        ...
        # 删除按钮
        button_course_delete = tk.Button(frame_course_check_result, text="删除", width=8, height=1,
...
def create_frame_course_delete(self):
    ...
    canvas_course_del = tk.Canvas(self.root, width=800, height=1200, scrollregion=(0, 0, 800, 1200))
    frame_check = tk.Frame(canvas_course_del, width=800, height=1200)
    self.frame_list["frame_course_delete"] = canvas_course_del
    frame_check.pack(side='top', anchor='n')
    canvas_course_del.create_window(400, 0, anchor='n', window=frame_check)
