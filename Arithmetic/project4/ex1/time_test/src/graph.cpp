...
int main(){
    ...
    int vnum = 27;
    for(int j = 2; j <= 2; j++){
        for(int i = 27; i <= 729; i+=27){
            fin.open("../input/input" + to_string(i) + "_" + to_string(j) + ".txt");
            fout.open("../output/output" + to_string(i) + "_" + to_string(j) + ".txt");
            Graph G(vnum+1);
            while(!fin.eof()){
                int u, v, w;
                fin >> u >> v >> w;
