...
RISCV_TOOLCHAIN_PATH = '/opt/riscv32-linux/bin/'
...
if len(sys.argv) != 3:
    ...
    res = os.system( '%sriscv32-unknown-linux-gnu-as %s            -o compile_tmp.o   -march=rv32i' % (RISCV_TOOLCHAIN_PATH, INPUT) )
    if res != 0:
        print('\n    Assembling Error!')
        sys.exit()
    os.system( '%sriscv32-unknown-linux-gnu-ld compile_tmp.o -o compile_tmp.om'               % (RISCV_TOOLCHAIN_PATH       ) )
    os.system( 'rm compile_tmp.o'   )
    os.system( '%sriscv32-unknown-linux-gnu-objcopy -O binary compile_tmp.om compile_tmp.bin' % (RISCV_TOOLCHAIN_PATH,      ) )
