...
std::string CallInst::print()
{
    ...
    instr_ir += print_as_op(this->get_operand(0), false);
    instr_ir += "(";
    for (int i = 1; (unsigned int)i < this->get_num_operand(); i++)
    {
        if( i > 1 )
            instr_ir += ", ";
...
{
    set_operand(0, ptr);
    for (int i = 0; (unsigned int)i < idxs.size(); i++) {
...
Type *GetElementPtrInst::get_element_type(Value *ptr, std::vector<Value *> idxs)
{
    ...
    if (ty->is_array_type())
    {
        ...
        for (int i = 1; (unsigned int)i < idxs.size(); i++) {
            ty = arr_ty->get_element_type();
            if ((unsigned int)i < idxs.size() - 1) {
#ifdef DEBUG
...
std::string GetElementPtrInst::print()
{
    ...
    instr_ir += ", ";
    for (int i = 0; (unsigned int)i < this->get_num_operand(); i++)
    {
        if( i > 0 )
            instr_ir += ", ";
        instr_ir += this->get_operand(i)->get_type()->print();
...
{
    for ( int i = 0; (unsigned int)i < vals.size(); i++)
    {
...
std::string PhiInst::print()
{
    ...
    instr_ir += this->get_operand(0)->get_type()->print();
    instr_ir += " ";
    for (int i = 0; (unsigned int)i < this->get_num_operand()/2; i++)
    {
        if( i > 0 )
            instr_ir += ", ";
        instr_ir += "[ ";
